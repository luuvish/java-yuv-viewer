...
public class YuvFrame extends JFrame {
  ...
  private void defaultScale() {
    ...
    spinnerScaleX.setValue(convertScaletoString(scaleX));
    spinnerScaleY.setValue(convertScaletoString(scaleY));
  }
  ...
  //  canvas.repaint();
  //  sliderFramePos.setValueIsAdjusting(true);
  //  sliderFramePos.setValue((int)framePos);
  //  sliderFramePos.setValueIsAdjusting(false);
  }
  ...
  public void step(long stepSize) {
    ...
    if (!reader.isLastPos() && stepSize > 0) {
      long framePos = reader.getFramePos() + stepSize;
      if (framePos > reader.getFrameNum()) framePos = reader.getFrameNum()-1;
      changePos(framePos);
    }
  }
  ...
  public void jump(boolean last) {
    if (!last) {
      if (!reader.isFirstPos()) changePos(0);
    } else {
      if (!reader.isLastPos()) changePos(reader.getFrameNum()-1);
    }
  }
}